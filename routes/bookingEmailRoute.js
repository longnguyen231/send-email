...
router.post('/send-email', async (req, res) => {
  try {
    ...
    const nights =
      ci && co
        ? Math.max(1, Math.round((co - ci) / (1000 * 60 * 60 * 24)))
        : null;
    ...
    const roomsArray = (() => {
      ...
      if (fallbackRoom && typeof fallbackRoom === 'object')
        return [fallbackRoom];
      if (
        req.body.roomName ||
        req.body.roomType ||
        ...
        const itemNights =
          Number(bookingData.nights ?? req.body.nights ?? nights ?? 1) || 1;
        const name = req.body.roomName ?? req.body.roomType;
        return [
          { name, quantity: qty, pricePerNight: ppn, nights: itemNights },
        ];
      }
      return [];
    })();
    ...
    const roomSection = `
      ...
      ${
        overallTotal != null
          ? `<p><b>Total Amount:</b> ${fmtMoney(overallTotal)}</p>`
          : ''
      }
    `;
